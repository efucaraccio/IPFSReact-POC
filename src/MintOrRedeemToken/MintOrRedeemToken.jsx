...
const MintOrRedeemToken = ({ token, ...props }) => {
  /* Context props */
  const { convertToken, mocState, userState, mocStatePrices, loading } = props;
  ...
  const calcInterests = async newValueYouExchange => {
    ...
    if (actionIsMint && currencyYouReceive === 'RISKPROX') {
      ...
      interestRate = formatVisibleValue(interestRate, 'commissionRate', TAPi18n.getLanguage());
    }
  ...
  const onConfirmTransactionFinish = async () => {
    ...
    exchangeMethod(userComment, formatValueWithContractPrecision(valueYouExchange, 'RESERVE'));
    setConfirmingTransaction(false);
  };
  ...
  const setAllowance = allowanceEnabled => {
    ...
    const result = window.nodeManager.approveMoCToken(allowanceEnabled, (a, _txHash) => {
      msgAllowanceTx(_txHash);
    });
  ...
  const renderFooter = () => {
    return (
      <div className="MintOrRedeemTokenFooter AlignedAndCentered">
        ...
        <div className={`MainActions ${window.appMode} AlignedAndCentered`}>
          <ButtonSecondary
  ...
  return (
    <Card
      title={
        actionIsMint ? t('global.MintOrRedeemToken_Mint') : t('global.MintOrRedeemToken_Redeem')
      }
      loading={loading}
...
export default tokenPricesContainer(mocStateContainer(userStateContainer(MintOrRedeemToken)));
